...
| L2 真实数据回放 | C++ | 2026-06-17 沪深 20 只股票逐笔成交 → 峰值 **3.56M QPS** |
...
| 日期 | 2026-06-17（最近交易日） |
| 标的 | 平安银行、贵州茅台、招商银行等 20 只沪深股票 |
| 原始交易 | 69,633 条逐笔成交 |
| 生成订单 | 80,217 笔（NEW + ~20% CANCEL） |
| 回放方式 | C++ 多连接全速发送（benchmark/l2_replay.cpp） |
| 结果 | 48,000 笔成功，峰值 **3.56M QPS** |
| 失败原因 | 客户端单线程分批收发，TCP 收缓冲积压导致 RST。改用独立收发线程（如 benchmark_client）可 100% 完成 |

与 Python 回放（~191K QPS）对比，C++ 回放将 QPS 提升 18 倍，说明 191K 是 Python 语言瓶颈而非服务端瓶颈。服务端在真实数据分布下同样达到百万 QPS。
